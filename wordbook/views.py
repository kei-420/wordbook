...
class RepeatedGameView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        is_randomly_selected = np.random.choice(
            list(user_word_data.values('word__vocab', 'word_meaning__vocab_meaning')),
            1,
            replace=False,
        )
        # for内では特定の要素の取得
        for j in is_randomly_selected:
            ...
            # 答えである意味
            is_answer = j['word_meaning__vocab_meaning']

        # ４つの選択肢のうち、答え以外の３つの取得かつ、その３つが答えと重複しないようにした。
        choices = WordMeanings.objects.exclude(vocab_meaning=is_answer).filter(lang='jpn').values('vocab_meaning')
        # リストからランダム抽出。重複なし。
